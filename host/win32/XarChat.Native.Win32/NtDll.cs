...
namespace XarChat.Native.Win32
{
    ...
    {
        public static IEnumerable<nint> EnumerateChildProcesses(
            nint processId,
            bool allDescendants = false)
        {
            var parentInfo = NtQueryInformationProcess(processId);
            if (parentInfo is null)
            {
                yield break;
            }

            var parentUniqueProcessId = parentInfo.UniqueProcessId;

            foreach (var cp in PSAPI.EnumProcesses())
            {
                ProcessBasicInformation? pbi = null;
                try
                {
                    pbi = NtQueryInformationProcess((nint)cp);
                }
                catch { }
                if (pbi?.InheritedFromUniqueProcessId == parentUniqueProcessId)
                {
                    yield return (nint)cp;
                    if (allDescendants)
                    {
                        foreach (var ccp in EnumerateChildProcesses((nint)cp))
                        {
                            yield return ccp;
                        }
                    }
        ...
        public unsafe static ProcessBasicInformation? NtQueryInformationProcess(nint processId)
        {
            ...
            if (phandle.Value != 0)
            {
                ...
                {
                    uint neededLen = 0;
                    uint currentAllocLen = (uint)Marshal.SizeOf<PROCESS_BASIC_INFORMATION>();
                    nint ptrPbi = Marshal.AllocHGlobal((int)currentAllocLen);
                    try
                    {
                        ...
                        if (res == 0)
                        {
                            PROCESS_BASIC_INFORMATION* resPbi = (PROCESS_BASIC_INFORMATION*)ptrPbi;
                            var result = new ProcessBasicInformation()
                            {
                                ExitStatus = resPbi->ExitStatus,
                                AffinityMask = resPbi->AffinityMask,
                                BasePriority = resPbi->BasePriority,
                                InheritedFromUniqueProcessId = resPbi->InheritedFromUniqueProcessId,
                                PebBaseAddress = resPbi->PebBaseAddress,
                                UniqueProcessId = resPbi->UniqueProcessId
                            };
                            return result;
                        }
                        else
                    ...
                    {
                        Marshal.FreeHGlobal(ptrPbi);
                    }
                }
                finally
                {
                    Windows.Win32.PInvoke.CloseHandle(phandle);
                }
            }
            else
            {
        ...
        private unsafe static void ZeroMemory(byte* ptrPbi, uint neededLen)
        {
        ...
        public class ProcessBasicInformation
        {
            ...
            public nint PebBaseAddress { get; init; }

            public nuint AffinityMask { get; init; }

            public int BasePriority { get; init; }

            public nuint UniqueProcessId { get; init; }

            public nuint InheritedFromUniqueProcessId { get; init; }
        }
