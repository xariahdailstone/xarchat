...
namespace XarChat.Backend.Features.EIconLoader.Impl
{
    internal class EIconLoaderImpl : IEIconLoader
    {
        ...
        public async Task<XarChatProtocolResponse> GetEIconAsync(
            string name, 
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (findResult.Headers.TryGetValue("Not-Found", out var nf) && nf == "true")
            {
                ...
                return XarChatProtocolResponse.NotFound;
            }
            ...
            void includeHeader(string headerName)
            {
                ...
                {
                    respHeaders.Add(new(headerName, hdrValue));
                    //httpContext.Response.Headers[headerName] = hdrValue;
                }
            }

            respHeaders.Add(new("Date", DateTime.UtcNow.ToString("R")));
            ...
            respHeaders.Add(new("Content-Type", findResult.Headers["Content-Type"]));
            return new XarChatProtocolResponse(200, "OK", respHeaders, respStream);
        }

        private static int _nextBackendRequestId = 1;
        private async Task<(Stream Stream, Dictionary<string, string> Headers)> GetEIconFromFListAsync(
