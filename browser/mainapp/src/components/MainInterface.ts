...
import { ContextMenuItem, ContextMenuItemBuilder, ContextMenuUtils } from "../util/ContextMenuUtils.js";
...
export class MainInterface extends ComponentBase<AppViewModel> {
    constructor() {
        ...
        this.addEventListener("contextmenu", (ev) => {
            ...
            if (sel?.type == "Range") {
                ContextMenuUtils.add(ContextMenuItemBuilder.createCopy());
            }
        }, true);
        this.addEventListener("contextmenu", (ev) => {
            const vm = this.viewModel;
            if (vm && !ev.defaultPrevented) {
                ev.preventDefault();
                if (ContextMenuUtils.hasMenuItems) {
                    (async () => {
                        const cvm = await ContextMenuUtils.makePopupViewModelAsync(vm, ev.clientX, ev.clientY);
                        vm.popups.add(cvm);
                    })();
                }
            }
